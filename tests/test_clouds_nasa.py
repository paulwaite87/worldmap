...
class MockNasaCloudUpdater(NasaCloudUpdater):
...
def test_nasa_clouds_pipeline(test_env):
    updater = MockNasaCloudUpdater(test_env["config"], test_env["map_data"])
    ...
    with patch("worldmap.tasks.clouds_nasa.urllib.request.urlopen") as mock_urlopen:
    ...
    ), "NASA Clouds JPEG failed verification!"